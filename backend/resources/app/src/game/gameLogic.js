...
class GameLogic {
    constructor(wsNumber, config, addLogCallback, updateConfigCallback, reconnectCallback) {
        ...
        this.opponentExactTiming = null; // Opponent's EXACT timing captured from prison message
        this.opponentTimingCaptured = false; // Flag to track if opponent timing was captured
    ...
    getContextFromPing() {
        ...
        if (ping === null) {
            return 'NORMAL'; // Default if no ping data
        }

        // Context thresholds (can be adjusted)
        if (ping < 50) {
            return 'FAST';
        } else if (ping > 150) {
            return 'SLOW';
    ...
    async ensurePingMeasured() {
        ...
        if (pingMs !== null) {
            ...
            this.addLog(this.wsNumber, `📡 Ping: ${pingMs}ms (${context})`);
        } else {
    ...
    async recordDefenseMetric() {
        try {
            ...
            const rivalName = this.rivalWhoKickedUs || 'Unknown';
            ...
            await this.recordImprisonmentMetric(
                ...
                'primary',                           // codeUsed
                false,                               // isClanMember
                Date.now() - this.connectionStartTime,  // timestampMs
                false,                               // isSuccess (you got kicked = failure)
                this.mlTimingWhenKicked,             // timingValue (YOUR ACTION 3 timing used)
                'defense',                           // timingType
                pingMs,                              // pingMs
                context,                             // context
            ...
            // Notify AI Core if enabled
            if (this.aiEnabled && this.aiCore) {
                this.aiCore.recordRivalAttack(this.mlTimingWhenKicked);
            }
            ...
            this.opponentExactTiming = null;
            this.opponentTimingCaptured = false;
    ...
    getTiming(mode) {
        ...
        if (this.aiEnabled && this.aiCore && this.aiCore.currentTiming) {
            ...
            if (this.aiCore.getTimingWithJitter) {
                ...
                fileLogger.aiStatus(this.wsNumber, `AI timing: ${timing}ms (${mode})`);
                return timing;
            }
    ...
    async initializeAI(supabaseUrl, supabaseKey) {
        try {
            ...
            this.addLog(this.wsNumber, `🧠 Simple AI: ${this.aiCore.currentTiming}ms`);
    ...
    async learnFromResult(result, opponentLeftTime = null) {
        ...
        try {
            ...
            const timingType = this.status === "attack" ? 'attack' : 'defense';

            // Track opponent leaving time if available
            if (this.aiCore.trackOpponentLeaving && opponentLeftTime) {
                this.aiCore.trackOpponentLeaving(opponentLeftTime, resultType);
            }
            ...
            try {
                ...
                this.addLog(this.wsNumber, `🧠 AI: ${stats.successRate}% success, next ${nextTiming}ms`);
            } catch (statsError) {
                DEBUG && console.log(`[WS${this.wsNumber}] 🧠 AI learned: ${resultType} (${timingType}) → Next: ${nextTiming}ms`);
                this.addLog(this.wsNumber, `🧠 AI: Next ${nextTiming}ms`);
            }
    ...
    _adjustTiming(key, increment) {
        ...
        if (increment) {
            ...
            if (value > maxBound) {
                ...
                this.addLog(this.wsNumber, `🔄 AUTO INTERVAL: Cycled from max (${maxBound}ms) back to starting point (${startingPoint}ms)`);
            ...
            if (value < minBound) {
                ...
                this.config[key] = value;
                this.updateConfig(key, value);
                this.addLog(this.wsNumber, `🔄 AUTO INTERVAL: Cycled from min (${minBound}ms) back to starting point (${startingPoint}ms)`);
     ...
     */
    addToSmartModePool(ws, rival, source = "JOIN", messageArrivalTime = Date.now()) {
    ...
    scheduleSmartModeAttack(ws, target, waitTime, timingMode = "attack") {
        ...
        this.attackTimeout = setTimeout(async () => {
            ...
            if (ws.readyState === ws.OPEN) {
                // 🧠 AI CORE: Re-measure ping if needed (every 3 imprisonments)
                if (this.aiEnabled && this.aiCore) {
                    try {
                        await this.aiCore.detectContext(ws, false); // false = only re-measure if counter reached
                    } catch (error) {
                        console.error(`[WS${this.wsNumber}] ❌ Failed to re-measure ping:`, error.message);
                    }
                }
                ...
                // Increased from 75ms to 150ms to account for network latency
                ...
                setTimeout(() => {
                    ...
                    if (this.pending850Result === 'SUCCESS') {
                        isSuccess = true;
                        adjustmentReason = null; // Will be fetched from AI
                    } else if (this.pending850Result === '3S_ERROR') {
                        isSuccess = false;
                        adjustmentReason = null; // Will be fetched from AI
                    } else {
                        // No 850 response received in 150ms - mark as null (unknown)
                        isSuccess = null;
                        adjustmentReason = 'NO_850_RESPONSE';
                        DEBUG && console.log(`[WS${this.wsNumber}] ⚠️ No 850 response in 150ms - recording with null status`);
                    }
                    ...
                    this.pending850Result = null;
                    this.pending850AlreadyRecorded = false;  // Reset flag

                    // Now send QUIT
                    if (ws.readyState === ws.OPEN) {
                        ws.send(`QUIT :ds\r\n`);
                        this.addLog(this.wsNumber, `🚪 QUIT after imprison (150ms delay)`);
                    }
    ...
    async promoteBackupToPrimary(ws) {
        ...
        // 🧠 AI CORE: Update rival name for new primary target (MUST AWAIT!)
        if (this.aiEnabled && this.aiCore && this.aiCore.setRivalName) {
            await this.aiCore.setRivalName(this.backupTarget.username);

            // Reset LEFT_EARLY tracking for new rival
            if (this.aiCore.opponentIntelligence) {
                this.aiCore.opponentIntelligence.opponentLeavingTimes = [];
                DEBUG && console.log(`[WS${this.wsNumber}] 🔄 Reset LEFT_EARLY tracking for new rival`);
            }
        }
        ...
        DEBUG && console.log(`[WS${this.wsNumber}] 📊 Adjusted wait time: ${finalWaitTime}ms`);
    ...
    handle353Message(ws, snippets, text, messageArrivalTime = Date.now()) {
        ...
        if (this.config.modena === true) {
            ...
            if (this.inPrison || this.prisonConfirmed || this.escapeInProgress) {
                ...
                this.addLog(this.wsNumber, `⏸️ In prison - skipping rival detection`);
        ...
        try {
            ...
            if (usersToBan.length > 0) {
                ...
                usersToBan.forEach((user, index) => {
                    ...
                    }, index * 100); // Stagger bans by 100ms to avoid rate limiting
    ...
    handle353KickMode(ws, snippets, text, messageArrivalTime = Date.now()) {
        try {
            const channelName = snippets[3];
            ...
            if (usersToAct.length > 0) {
                ...
                if (isKickMode) {
                        ...
                        const innerTimeout = setTimeout(() => {
                            if (ws.readyState === ws.OPEN) {
                                ...
                                if (index === usersToAct.length - 1) {
                                    DEBUG && console.log(`[WS${this.wsNumber}] Kick mode - Staying connected to kick more users`);
                                    this.addLog(this.wsNumber, `✅ Staying connected (Kick mode)`);
                                }
                            }
                        }, index * 100); // Stagger kicks by 100ms to avoid rate limiting
                    ...
                    usersToAct.forEach(user => {
                        ...
                        }, "353", messageArrivalTime);  // ✅ Pass messageArrivalTime for accurate timing
                    });
    ...
    handleJoinMessage(ws, snippets, text, messageArrivalTime = Date.now()) {
        ...
        if (this.config.modena === true) {
            ...
            if (this.inPrison || this.prisonConfirmed || this.escapeInProgress) {
                ...
                this.addLog(this.wsNumber, `⏸️ In prison - skipping new joiner`);
                return; // Exit early - don't process rivals
            }
    ...
    handleJoinKickMode(ws, snippets, text, messageArrivalTime = Date.now()) {
        try {
            ...
            if (shouldAct) {
                if (isKickMode) {
                    ...
                    this.addToSmartModePool(ws, {
                        userid: userid,
                        username: username
                    }, "JOIN", messageArrivalTime);  // ✅ Pass messageArrivalTime for accurate timing  // ✅ Pass "JOIN" as source (uses attack timing)
        ...
        this.timeout = setTimeout(async () => {
            ...
            if (ws.readyState === ws.OPEN) {
                ...
                if (this.config.modena) {
                    ...
                    DEBUG && console.log(`[WS${this.wsNumber}] 🎯 Attacking: ${name} (detected at ${this.rivalDetectedTime})`);
                }
        ...
        try {
            ...
            if (hasPrisonWarning) {
                ...
                if (this.actionSentTime && this.rivalDetectedTime && this.currentTargetName && !this.mlTimingWhenKicked) {
                    ...
                    DEBUG && console.log(`[WS${wsNumber}] ✅ KICKED by rival: ${this.rivalWhoKickedUs}`);
                    DEBUG && console.log(`[WS${wsNumber}] ✅ ACTION 3 timing used: ${this.mlTimingWhenKicked}ms`);

                    // 🎯 CRITICAL: Check if we captured opponent's exact timing from prison message
                    if (this.opponentTimingCaptured && this.opponentExactTiming) {
                        DEBUG && console.log(`[WS${wsNumber}] 🎯 Opponent EXACT timing available: ${this.opponentExactTiming}ms`);

                        // ✅ Learn from KICKED result with EXACT opponent timing
                        if (this.aiEnabled && this.aiCore) {
                            this.aiCore.trackOpponentLeaving(this.opponentExactTiming, 'KICKED');
                            DEBUG && console.log(`[WS${wsNumber}] 📊 Tracked KICKED with exact opponent timing: ${this.opponentExactTiming}ms`);
                        }
                    } else {
                        DEBUG && console.log(`[WS${wsNumber}] ⚠️ Opponent exact timing NOT captured - using estimation`);

                        // ✅ Learn from KICKED result (for anti-baiting) - will use estimation
                        if (this.aiEnabled && this.aiCore && this.aiCore.trackOpponentLeaving) {
                            // For KICKED, we don't know exact opponent leaving time
                            // But we know they kicked us, so pass null for leaving time
                            this.aiCore.trackOpponentLeaving(null, 'KICKED');
                            DEBUG && console.log(`[WS${wsNumber}] 📊 Tracked KICKED result for anti-baiting (estimation)`);
                        }
                    }

                    // ✅ RECORD DEFENSE METRIC IMMEDIATELY (don't wait for escape)
                    DEBUG && console.log(`[WS${wsNumber}] 📊 Recording defense metric immediately...`);
                    this.recordDefenseMetric().catch(err => {
                        console.error(`[WS${wsNumber}] Failed to record defense metric:`, err);
                    });
                    ...
                    this.rivalWhoKickedUs = this.lastTargetName || this.currentTargetName || 'Unknown';
                    this.wasKickedToPrison = true;
                    DEBUG && console.log(`[WS${wsNumber}] ✅ ML timing when kicked (fallback): ${this.mlTimingWhenKicked}ms`);
                    ...
                    DEBUG && console.log(`[WS${wsNumber}] ⚠️ Cannot track KICKED - conditions not met`);
                    if (!this.aiEnabled) DEBUG && console.log(`[WS${wsNumber}]    → AI not enabled`);
                    if (!this.aiCore) DEBUG && console.log(`[WS${wsNumber}]    → AI Core not initialized`);
                    if (!this.actionSentTime) DEBUG && console.log(`[WS${wsNumber}]    → No ACTION 3 sent`);
                    if (!this.rivalDetectedTime) DEBUG && console.log(`[WS${wsNumber}]    → No rival detected time`);
                    if (!this.currentTargetName) DEBUG && console.log(`[WS${wsNumber}]    → No current target name`);
                    if (this.mlTimingWhenKicked) DEBUG && console.log(`[WS${wsNumber}]    → Already recorded (${this.mlTimingWhenKicked}ms)`);
                }
            ...
            if (this.config.imprisonmode) {
                ...
                if (userid === this.primaryTarget.userid) {
                    ...
                    if (!this.actionSentTime && this.currentTargetName === this.primaryTarget.username) {
                        ...
                        // Learn from early leave (pass rival's leaving time for baiting detection)
                        ...
                        if (this.rivalDetectedTime) {
                            ...
                            if (rivalActualLeaveTime < absoluteMinimum) {
                                ...
                                const timingValue = this.getTiming(timingType);
                                ...
                                this.recordImprisonmentMetric(
                                    ...
                                    codeUsed,
                                    isClanMember,
                                    Date.now() - this.connectionStartTime,  // timestampMs = connection time
                ...
                // If BACKUP left
                    ...
                    if (this.backupTarget.appearanceTime) {
                        ...
                        // Learn from backup early leave
                        this.learnFromResult('LEFT_EARLY', backupActualLeaveTime).catch(err => {
                ...
                if (userid === this.primaryTarget.userid) {
                    if (isFounder) {
                        ...
                        if (!this.actionSentTime && this.currentTargetName === this.primaryTarget.username) {
                            ...
                            // Learn from early sleep (pass rival's sleep time for baiting detection)
    ...
    async handle850Message(ws, snippets, text, messageArrivalTime = Date.now()) {
        ...
        try {
            ...
            if (is3SecondError) {
                ...
                // ✅ Set flag to prevent 150ms timeout from recording duplicate
                this.pending850Result = '3S_ERROR';
                this.pending850AlreadyRecorded = true;

                // 🧠 AI CORE: Learn from 3S error
                await this.learnFromResult('3S_ERROR');

                // ✅ Record 3S_ERROR to database (same pattern as timer shift)
                ...
                if (this.config.timershift && !this.aiEnabled) {
                    ...
                    if (this.status === "attack") {
                        ...
                        this.addLog(this.wsNumber, `📊 Adjusting attack timing`);
                        ...
                        this.addLog(this.wsNumber, `📊 Adjusting defense timing`);
                        const newValue = parseInt(this.config[`waiting${this.wsNumber}`] || 1910);
                        fileLogger.autoInterval(this.wsNumber, oldValue, newValue, '3s error');
                    } else {
                        this.incrementAttack();
                        this.addLog(this.wsNumber, `📊 Adjusting attack timing (default)`);
                        const newValue = parseInt(this.config[`attack${this.wsNumber}`] || 1940);
                        fileLogger.autoInterval(this.wsNumber, oldValue, newValue, '3s error');
                    }
                }
            ...
            if (isSuccess) {
                ...
                // 🧠 AI CORE: Learn from success
                await this.learnFromResult('SUCCESS');

                // ✅ Record SUCCESS to database (same pattern as timer shift)
                DEBUG && console.log(`[WS${this.wsNumber}] 📊 Recording metric: SUCCESS`);
                if (this.rivalDetectedTime && this.currentTargetName && this.actionSentTime) {
                    const timestampMs = this.actionSentTime - this.rivalDetectedTime;
                    const codeUsed = this.currentCodeType || 'primary';
                    const isClanMember = this.isUserInBotGang(this.currentTargetName);
                ...
                if (this.config.timershift && !this.aiEnabled) {
                    ...
                    if (this.status === "attack") {
                        ...
                        this.decrementAttack();
                        this.addLog(this.wsNumber, `📊 Optimizing attack timing (default)`);
                        const newValue = parseInt(this.config[`attack${this.wsNumber}`] || 1940);
                        fileLogger.autoInterval(this.wsNumber, oldValue, newValue, 'success');
                    }
                }
        ...
        if (this.aiEnabled && this.aiCore) {
            ...
            if (!this.mlTimingWhenKicked) {
                ...
                if (this.actionSentTime && this.rivalDetectedTime && this.currentTargetName) {
                    ...
                    // Track KICKED for anti-baiting
                    if (this.aiCore.trackOpponentLeaving) {
                        this.aiCore.trackOpponentLeaving(null, 'KICKED');
                        DEBUG && console.log(`[WS${this.wsNumber}] 📊 Tracked KICKED result for anti-baiting`);
                    }
