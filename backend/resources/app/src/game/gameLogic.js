...
class GameLogic {
    constructor(wsNumber, config, addLogCallback, updateConfigCallback, reconnectCallback) {
        ...
        // Cycle timing instrumentation — tracks every segment of the cycle
        this.cycleTimer = { t999: null, tJoinSent: null, t353: null, tRivalDetected: null, tAction3: null, tQuit: null };
        ...
        // IMPORTANT: Restore inPrison from appState (persists across reconnections)
        const prisonKey = `inPrison${wsNumber}`;
        this.inPrison = appState.gameState[prisonKey] || false;
        DEBUG && console.log(`[WS${wsNumber}] Constructor - Restored inPrison: ${this.inPrison}`);
    ...
    parse353UserIds(text) {
        // Optimized: single pass, avoid regex on full string, fast numeric check
        var parts = text.split(" ");
        var result = [];
        for (var i = 0; i < parts.length; i++) {
            var item = parts[i];
            // Strip prefix chars (+, @, :) from start only
            while (item.length > 0 && (item[0] === '+' || item[0] === '@' || item[0] === ':')) {
                item = item.substring(1);
            }
            // Fast numeric check: length >= 6 and first char is digit
            if (item.length >= 6 && item[0] >= '0' && item[0] <= '9' && item !== '-') {
    ...
    async recordDefenseMetric() {
        try {
            ...
            // ✅ MARK AS RECORDED BEFORE await to prevent race condition
            // (escapeWithCode can run while recordImprisonmentMetric is awaiting)
            this.defenseMetricRecorded = true;

            // Clear defense tracking (before await to prevent duplicate reads)
            const kickedTiming = this.mlTimingWhenKicked;
            ...
            await this.recordImprisonmentMetric(
                ...
                this.currentCodeType || 'primary',   // codeUsed
                false,                               // isClanMember
                kickedTiming,                        // timestampMs = measured elapsed action timing
            ...
            DEBUG && console.log(`[WS${this.wsNumber}] 🛡️ Defense metric recorded: Bot used ${kickedTiming}ms timing against ${rivalName} and got KICKED`);
     ...
     */
    /**
     * Send QUIT and force-close after 50ms so we don't wait for server close
     */
    sendQuit(ws) {
        if (ws && ws.readyState === ws.OPEN) {
            ws.send("QUIT :ds\r\n");

            // Cycle timing: log full breakdown
            if (this.cycleTimer && this.cycleTimer.t999) {
                const ct = this.cycleTimer;
                ct.tQuit = Date.now();
                const total = ct.tQuit - ct.t999;
                const seg1 = ct.tJoinSent ? ct.tJoinSent - ct.t999 : '?';
                const seg2 = ct.t353 && ct.tJoinSent ? ct.t353 - ct.tJoinSent : '?';
                const seg3 = ct.tRivalDetected && ct.t353 ? ct.tRivalDetected - ct.t353 : '?';
                const seg4 = ct.tAction3 && ct.tRivalDetected ? ct.tAction3 - ct.tRivalDetected : '?';
                const seg5 = ct.tAction3 ? ct.tQuit - ct.tAction3 : '?';
                console.log(`[WS${this.wsNumber}] ⏱️ CYCLE: ${total}ms | 999→JOIN:${seg1}ms | JOIN→353:${seg2}ms | 353→RIVAL:${seg3}ms | RIVAL→ACTION3:${seg4}ms | ACTION3→QUIT:${seg5}ms`);
                fileLogger.log('CYCLE-TIMING', `total=${total}ms | 999→JOIN:${seg1} | JOIN→353:${seg2} | 353→RIVAL:${seg3} | RIVAL→ACTION3:${seg4} | ACTION3→QUIT:${seg5}`, this.wsNumber);
            }
    ...
    /**
     * Save inPrison flag to appState (persists across reconnections)
     * @param {boolean} value - Value to set
     */
    setInPrison(value) {
        this.inPrison = value;
        const { appState } = require("../config/appState");
        const prisonKey = `inPrison${this.wsNumber}`;
        appState.gameState[prisonKey] = value;
        DEBUG && console.log(`[WS${this.wsNumber}] setInPrison: ${value} (saved to appState)`);
    }
     ...
     */
    /**
     * Called when 850 arrives OR safety timeout fires after ACTION 3.
     * Records metric, clears state, sends QUIT.
     */
    _doPostActionQuit() {
        // Clear safety timeout (if 850 arrived early)
        if (this._pending850TimeoutId) {
            clearTimeout(this._pending850TimeoutId);
            this._pending850TimeoutId = null;
        }

        const ctx = this._postActionContext;
        if (!ctx) return;
        this._postActionContext = null;

        const { target, timingMode, waitTime, ws } = ctx;

        // Record metric if 850 gave us a result and not already recorded
        if (this.pending850Result && !this.pending850AlreadyRecorded) {
            const timestampMs = this.actionSentTime - this.rivalDetectedTime;
            const isSuccess = this.pending850Result === 'SUCCESS';

            DEBUG && console.log(`[WS${this.wsNumber}] 📊 Recording metric: ${this.pending850Result}`);
            this.recordImprisonmentMetric(
                target.username,
                this.currentCodeType || 'primary',
                this.isUserInBotGang(target.username),
                timestampMs,
                isSuccess,
                waitTime,
                timingMode,
    ...
    sendJoinWithRetry(ws, planet) {
        if (!ws || ws.readyState !== ws.OPEN) return;
        const joinCmd = planet ? `JOIN ${planet}\r\n` : `JOIN\r\n`;
        ws.send(joinCmd);
    }

    /**
     * Rejoin planet after escape with 1.5x backoff + retry (same pattern as reconnect)
     * @param {WebSocket} ws
     * @param {string} planet
     * @param {number} retryCount - current retry attempt
     */
    _rejoinPlanetWithRetry(ws, planet, retryCount) {
        const maxRetries = 3;
        const baseDelay = 2000; // Fixed 2s base delay for planet rejoin after escape
        const backoffMultiplier = Math.pow(1.5, retryCount);
        const maxBackoff = 15000;
        const backoffTime = Math.min(baseDelay * backoffMultiplier, maxBackoff);

        // Add jitter (±20%)
        const jitterRange = backoffTime * 0.2;
        const jitter = (Math.random() * jitterRange * 2) - jitterRange;
        const delay = Math.max(1500, Math.floor(backoffTime + jitter));

        this.addLog(this.wsNumber, `⏱️ Rejoining ${planet} in ${Math.floor(delay / 1000)}s${retryCount > 0 ? ` (retry ${retryCount}/${maxRetries})` : ''}`);
        DEBUG && console.log(`[WS${this.wsNumber}] Rejoin delay: base=${baseDelay}ms, backoff=${Math.floor(backoffTime)}ms, final=${delay}ms`);

        const timeoutId = setTimeout(() => {
            if (!ws || ws.readyState !== ws.OPEN) {
                DEBUG && console.log(`[WS${this.wsNumber}] ❌ WS not open for rejoin — skipping`);
                return;
            }
            if (!this.config.connected) {
                DEBUG && console.log(`[WS${this.wsNumber}] ❌ User disconnected — skipping rejoin`);
                return;
            }

            this.sendJoinWithRetry(ws, planet);
            this.addLog(this.wsNumber, `🔄 Rejoining ${planet}`);

            // Clear flag + reset escape state on first attempt
            this.setShouldRejoinPlanet(false);
            this.escapeInProgress = false;
            this.escapeRetryCount = 0;
            DEBUG && console.log(`[WS${this.wsNumber}] ✅ Escape complete - cleared escapeInProgress, reset retry counter`);

            // The 999 handler will confirm if JOIN succeeded.
            // If we get kicked again or see 403 "no such channel", the existing
            // handlers in message parsing will trigger rejoin automatically.
        }, delay);

        // Store so destroy() can clean it up
        if (this.innerTimeouts) {
            this.innerTimeouts.push(timeoutId);
        }
    }
    ...
    resetState() {
        ...
        // Note: Don't reset shouldRejoinPlanet or inPrison here - they persist until message 999;
    ...
    async learnFromResult(result, opponentLeftTime = null) {
        ...
        // A rival leaving at <1675ms is not a real timing signal — it corrupts ML state
        if (result === 'LEFT_EARLY' && opponentLeftTime !== null && opponentLeftTime < 1675) {
            fileLogger.log('AI-LEARN', `BLOCKED LEFT_EARLY: opponentLeftAt=${opponentLeftTime}ms < 1675ms (trap/bait) — ML not updated`, this.wsNumber);
            return;
        }
    ...
    async addToSmartModePool(ws, rival, source = "JOIN", messageArrivalTime = Date.now()) {
        ...
        if (!this.primaryTarget.userid) {
            ...
            if (this.cycleTimer && !this.cycleTimer.tRivalDetected) this.cycleTimer.tRivalDetected = Date.now();
    ...
    scheduleSmartModeAttack(ws, target, waitTime, timingMode = "attack") {
        ...
        // Drift-corrected timer: schedule 15ms early, then spin-wait for exact timing
        const scheduledAt = Date.now();
        const earlyMs = Math.min(15, waitTime);
        this.attackTimeout = setTimeout(async () => {
            // Spin-wait the remaining ms for precision (eliminates setTimeout jitter)
            const targetTime = scheduledAt + waitTime;
            while (Date.now() < targetTime) { /* spin */ }
            ...
            if (ws.readyState === ws.OPEN) {
                ...
                if (this.cycleTimer) this.cycleTimer.tAction3 = Date.now();
                ...
                // ✅ Event-driven: 850 handler will call _doPostActionQuit() immediately
                // Safety timeout = ping ms (max wait if 850 never arrives)
                this.pending850Response = true;
                this.pending850Result = null;
                this.pending850AlreadyRecorded = false;
                this._postActionContext = { target, timingMode, waitTime, ws };

                const maxWait = Math.max(100, this.getCurrentPing() || 150);
                DEBUG && console.log(`[WS${this.wsNumber}] ⏱️ Waiting for 850 (max ${maxWait}ms based on ping)`);

                this._pending850TimeoutId = setTimeout(() => {
                    this._pending850TimeoutId = null;
                    if (this.pending850Response) {
                        DEBUG && console.log(`[WS${this.wsNumber}] ⚠️ No 850 in ${maxWait}ms — QUIT without result`);
                        this.pending850Response = false;
                        this.pending850AlreadyRecorded = false;
                        this._doPostActionQuit();
                    }
                }, maxWait);

                // DON'T set shouldRejoinPlanet for IMPRISON - bot should go to Prison first
            }
        }, Math.max(0, waitTime - earlyMs));
    ...
    handle353Message(ws, snippets, text, messageArrivalTime = Date.now()) {
        ...
        this.joinRetryCount = 0; // JOIN succeeded, reset retry counter

        DEBUG && console.log(`[WS${this.wsNumber}] 353 message received - Planet: ${planetName}`);

        // Cycle timing: mark 353 received
        if (this.cycleTimer && !this.cycleTimer.t353) this.cycleTimer.t353 = Date.now();
        ...
        if (planetName) {
            ...
            // Cancel the 5s fallback timer — 353 arrived, JOIN worked
            if (this._joinFallbackTimer) {
                clearTimeout(this._joinFallbackTimer);
                this._joinFallbackTimer = null;
                DEBUG && console.log(`[WS${this.wsNumber}] 353 arrived — cancelled JOIN fallback timer`);
            }
            ...
            if (isPrisonPlanet) {
                ...
                if (this.config.autorelease && !this.escapeInProgress) {
                    ...
                    setTimeout(async () => {
                        ...
                        if (success) {
                            // Clear escape flag BEFORE rejoin so 353 response is processed
                            this.escapeInProgress = false;
                            // Rejoin target planet after escape with backoff + retry
                            const targetPlanet = this.config.planet;
                            if (targetPlanet && ws.readyState === ws.OPEN) {
                                this._rejoinPlanetWithRetry(ws, targetPlanet, 0);
                            }
                        } else {
    ...
    async handleJoinKickMode(ws, snippets, text, messageArrivalTime = Date.now()) {
        try {
            ...
            // Skip any other own account (primary or alt codes from other connections)
            if (GameLogic.ownUserIds.has(userid)) {
                DEBUG && console.log(`[WS${this.wsNumber}] Skipping own account: ${userid}`);
                return;
            }
    ...
    handleJoinBanMode(ws, snippets, text, messageArrivalTime = Date.now()) {
        try {
            ...
            // Skip any other own account (primary or alt codes from other connections)
            if (GameLogic.ownUserIds.has(userid)) {
                DEBUG && console.log(`[WS${this.wsNumber}] Skipping own account in BAN mode: ${userid}`);
                return;
            }
    ...
    async startAttackSequence(ws, userid, name, mode, label) {
        ...
        const seqScheduledAt = Date.now();
        const seqEarlyMs = Math.min(15, timing);
        this.timeout = setTimeout(async () => {
            // Spin-wait for exact timing (drift correction)
            const seqTargetTime = seqScheduledAt + timing;
            while (Date.now() < seqTargetTime) { /* spin */ }
    ...
    handle332Message(ws, snippets, text, messageArrivalTime = Date.now()) {
        try {
            ...
            if (hasPrisonWarning) {
                // If already in prison, this is a duplicate 332 — skip recording
                if (this.prisonConfirmed) {
                    DEBUG && console.log(`[WS${this.wsNumber}] 332 duplicate — already in prison, skipping`);
                    return;
                }

                // CONFIRMED: We are in prison (first 332 for this event)
                this.prisonConfirmed = true;
                this.setInPrison(true);
                ...
                if (this.config.autorelease && !this.escapeInProgress) {
                    ...
                    setTimeout(async () => {
                        ...
                        if (success) {
                            ...
                            const targetPlanet = this.config.planet;
                            if (targetPlanet && !this.prisonConfirmed && ws.readyState === ws.OPEN) {
                                this._rejoinPlanetWithRetry(ws, targetPlanet, 0);
                                ...
                                DEBUG && console.log(`[WS${this.wsNumber}] ⚠️ WebSocket closed or no planet - cannot rejoin`);
                            }
                        } else {
            ...
            if (this.config.imprisonmode) {
                ...
                if (userid === this.primaryTarget.userid) {
                    ...
                    if (!this.actionSentTime && this.currentTargetName === this.primaryTarget.username) {
                        ...
                        // A rival leaving at <1675ms is a trap, not a real timing signal
                        if (rivalActualLeaveTime >= 1675) {
                            ...
                            fileLogger.log('AI-LEARN', `SKIP LEFT_EARLY: opponentLeftAt=${rivalActualLeaveTime}ms < 1675ms (trap/bait) — not feeding ML`, this.wsNumber);
                        }
                        ...
                        if (this.rivalDetectedTime) {
                            ...
                            // ✅ CRITICAL: Check if rival timing is above ABSOLUTE minimum (1675ms)
                            // Rival can be in ANY zone (SLOW/NORMAL/FAST) regardless of our ping
                            const absoluteMinimum = 1675;  // SLOW zone minimum (lowest valid timing)
                ...
                // If BACKUP left
                    ...
                    if (this.backupTarget.appearanceTime) {
                        ...
                        if (backupActualLeaveTime < 1675) {
                            fileLogger.log('AI-LEARN', `SKIP BACKUP LEFT_EARLY: opponentLeftAt=${backupActualLeaveTime}ms < 1675ms (trap/bait)`, this.wsNumber);
                        }
                        if (backupActualLeaveTime >= 1675) this.learnFromResult('LEFT_EARLY', backupActualLeaveTime).catch(err => {
            ...
            if (this.config.imprisonmode) {
                    ...
                    if (isFounder) {
                        ...
                        if (!this.actionSentTime && this.currentTargetName === this.primaryTarget.username) {
                            ...
                            if (rivalActualSleepTime >= 1675) {
                                ...
                                fileLogger.log('AI-LEARN', `SKIP SLEEP LEFT_EARLY: opponentLeftAt=${rivalActualSleepTime}ms < 1675ms (trap/bait)`, this.wsNumber);
                            }
                ...
                // If BACKUP sleeping
                    ...
                    if (this.backupTarget.appearanceTime) {
                        ...
                        // Learn from backup early sleep — skip trap/bait timings
                        if (backupActualSleepTime < 1675) {
                            fileLogger.log('AI-LEARN', `SKIP BACKUP SLEEP LEFT_EARLY: opponentLeftAt=${backupActualSleepTime}ms < 1675ms (trap/bait)`, this.wsNumber);
                        } else {
                        ...
                        // Record metric only if above absolute minimum (not trap/bait)
                        if (backupActualSleepTime >= 1675) {
                            const codeUsed = this.currentCodeType || 'primary';
                            const isClanMember = this.isUserInBotGang(this.backupTarget.username);
                            const timingType = this.backupTarget.source === "353" ? "defense" : "attack";
    ...
    async handle850Message(ws, snippets, text, messageArrivalTime = Date.now()) {
        ...
        try {
            ...
            if (is3SecondError) {
                ...
                // ✅ 850 arrived — QUIT immediately (event-driven, no waiting)
                if (this._postActionContext) {
                    this._doPostActionQuit();
                }
            }
            ...
            if (isSuccess) {
                ...
                DEBUG && console.log(`[WS${this.wsNumber}] SUCCESS CHECK: pending850Response=${this.pending850Response}, pending850AlreadyRecorded=${this.pending850AlreadyRecorded}`);

                // Skip ONLY if 150ms timeout already fired AND already recorded something
                if (!this.pending850Response && this.pending850AlreadyRecorded) {
                    DEBUG && console.log(`[WS${this.wsNumber}] Late 850 (SUCCESS) — already recorded by timeout, skipping`);
                    return;
                }
                ...
                DEBUG && console.log(`[WS${this.wsNumber}] SUCCESS RECORD: rivalDetectedTime=${this.rivalDetectedTime}, currentTargetName=${this.currentTargetName}, actionSentTime=${this.actionSentTime}`);
                if (this.rivalDetectedTime && this.currentTargetName && this.actionSentTime) {
                    const timestampMs = this.actionSentTime - this.rivalDetectedTime;
    ...
    async escapeAll() {
        ...
        if (success) {
            ...
            // Clear prisonConfirmed so escape flow doesn't re-trigger,
            // but keep inPrison=true until 353 confirms bot landed on non-prison planet.
            // This prevents the bot from attacking while still in prison waiting to rejoin.
            this.prisonConfirmed = false;
            this.currentPlanet = null;
        } else {
...
// Shared set of all own user IDs across all connections — used to prevent
// connections from targeting each other (e.g. primary vs alt code accounts)
GameLogic.ownUserIds = new Set();
