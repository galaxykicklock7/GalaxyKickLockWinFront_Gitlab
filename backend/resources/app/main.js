...
apiServer.use((req, res, next) => {
  ...
  if (isAllowed) {
    // Set specific origin if present, otherwise use * for non-origin requests
    if (origin) {
    ...
    res.setHeader('Vary', 'Origin'); // Ensure caches consider Origin header
  ...
  try {
    ...
    Object.keys(appState.websockets).forEach(key => {
      try {
        ...
        const currentPlanet = appState.gameLogic[logicKey] ? appState.gameLogic[logicKey].currentPlanet : undefined;
...
apiServer.get('/api/metrics/:connectionNumber', async (req, res) => {
  try {
    ...
    console.log(`[METRICS] Found ${(data && data.data && data.data.length) || 0} metrics for conn ${connNum}`);
