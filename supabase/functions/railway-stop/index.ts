// Supabase Edge Function: railway-stop
// Stops a user's Railway deployment and updates Supabase status
//
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Step 1: Look up user's railway_service_id
    ...
    if (dbError || !deployment?.railway_service_id) {
      // No service mapped — just mark as stopped
      await supabase
        .from("user_deployments")
      ...
      return new Response(
        JSON.stringify({ success: true, message: "No active service found, marked as stopped" }),
    ...
    // Step 5: Update Supabase status to stopped
    await supabase
      .from("user_deployments")
      .update({ status: "stopped", updated_at: new Date().toISOString() })
      .eq("user_id", user_id);
    ...
    return new Response(
      JSON.stringify({
        success: true,
        stopped_deployment: latestDeploy?.id || null,
        previous_status: latestDeploy?.status || "none",
      }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (error) {
